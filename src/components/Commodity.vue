...
<template >
  <div v-if="unRegister">
    <msg title="提示" :description="message+wxid" :buttons="buttons"  icon="warn"></msg>
  ...
  <div style="padding-bottom: 50px" v-else>
     ...
     <div class="food_container">
       ...
       <x-header :left-options="{showBack: false}" class="header">热销商品
         </x-header>
        <a  class="iconfont" :class="{buyActive:total>0}" @click="buy">结算</a>
      <div v-transfer-dom>
              ...
              <section v-show="sortBy == 'food'" class="category_container sort_detail_type">
                ...
                <section class="category_right">
                  <ul>
                    ...
                    class="category_right_li" @click="getCategoryIds(item.typeCode, item.type)"
      ...
      <section class="buy_cart_container">
          <section @click="toggleCartList" class="cart_icon_num">
              <div class="cart_icon_container" :class="{cart_icon_activity: total > 0, move_in_cart:receiveInCart}" ref="cartContainer">
                  <span v-if="total" class="cart_list_length">
                      {{total}}
                  </span>
                  <svg class="cart_icon">
      ...
      <section class="cart_food_list" v-show="showCartList&&total>0">
          ...
          <section class="cart_food_details" id="cartFood">
              <ul>
                  <li v-for="(item, index) in goodsAllList" v-if="item.number>0" :key="index" class="cart_food_li">
                      ...
                      <section class="cart_list_control">
                          <span @click="removeOutCart(item)">
                              <svg>
                                  <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#cart-minus"></use>
                              </svg>
                          </span>
                          <span class="cart_num">{{item.number}}</span>
      ...
      <group  >
        ...
        <goods-list   :list="goodslist" @on-click-img="imgclick($event)" @on-swap-click="history($event)" :showImg="showImg"　 class="goodslist"></goods-list>
      </group>
    ...
    <div v-transfer-dom>
      <x-dialog v-model="showToast" class="dialog-demo">
        <form-preview header-label="金额" class="formpreview"  :header-value="'¥'+totalPrice" :body-items="showlist" ></form-preview>
    ...
    <div v-transfer-dom>
      <x-dialog v-model="showHistory" class="dialog-demo">
        <group >
          <group-title slot="title">最近一周收货历史<span @click="showHistory=false" style="float:right;" class="vux-close"></span></group-title>
...
export default {
  ...
  methods: {
        ...
        HttpUtil.get(
          ...
          function(data) {
            data.forEach(function(item) {
              if (item.hasImg == "1") {
                item.src = "../goodsImg/" + item.goodCode + ".jpg";
                item.bigSrc = "../artwork/" + item.goodCode + ".jpg";
              } else {
                item.src = "http://iph.href.lu/100x100?text=济胜软件";
                item.bigSrc = "http://iph.href.lu/100x100?text=济胜软件";
              }
    ...
    getGoodsTypeList() {
      let _this = this;
      HttpUtil.get("goods/getAllGoodsType.do", {}, function(data) {
